...
def recommend_commands_based_on_query(query: str) -> list:
    ...
    if any(k in q for k in ["cpu", "memory", "disk"]):
        # For CPU-focused queries, prefer a short process list sorted by CPU
        cmds = [
...
def _extract_block_for_command(full_stdout: str, cmd: str) -> str:
    """
    The ssh/ssm output is formatted with command separators like:
    $ <cmd>\n<output>\n$ <next cmd> ...
    This helper extracts the output block for the given `cmd` string.
    """
    ...
    for i, L in enumerate(lines):
        if inside:
            if L.startswith("$ "):
                break
            out_lines.append(L)
        elif L.strip().startswith(prefix):
            # the actual output begins on the next line
            inside = True
            # if there are output lines immediately following, include them
    return "\n".join(out_lines).strip()


def _parse_ps_top_processes(block: str, max_rows: int = 4):
    """
    Parse the `ps -eo pid,comm,%cpu` output block and return a list of (cmd, cpu_float).
    """
    procs = []
    if not block:
        return procs
    for line in block.splitlines():
        ...
        if not line:
            continue
        # Skip header lines if present (e.g., "PID COMMAND %CPU")
        if line.lower().startswith("pid") or line.lower().startswith("pid,comm"):
            continue
        # columns separated by whitespace; COMMAND may not contain spaces here
        parts = line.split()
        if len(parts) >= 3:
            # last column is %cpu
            try:
                ...
                cpu = 0.0
            cmd = parts[1] if len(parts) >= 3 else parts[0]
            procs.append((cmd, cpu))
    # keep top N
    return procs[:max_rows]


def _render_cpu_bars(area, procs):
    """
    Render a simple HTML bar chart for processes using inline styles.
    `procs` is list of (cmd, cpu)
    """
    if not procs:
        area.info("No process info available to render chart.")
        return
    max_cpu = max((p[1] for p in procs), default=1)
    html = ["<div style='font-family:monospace;max-width:600px'>"]
    for cmd, cpu in procs:
        pct = min(100, cpu)
        width = int((pct / max_cpu) * 100) if max_cpu > 0 else int(pct)
        html.append(f"<div style='margin:6px 0'><strong>{cmd}</strong> <small>({cpu:.1f}%)</small>")
        html.append(f"<div style='background:#e6eef6;border-radius:6px;height:14px;width:100%;'><div style='background:#7c3aed;height:14px;border-radius:6px;width:{width}%;'></div></div></div>")
    html.append("</div>")
...
def main():
    ...
    with left_col:
        ...
        with st.form(key="server_form", clear_on_submit=True):
            add_input = st.text_input("Add server (format: name,host,instance_id) — only host/name required", placeholder="my-server,34.204.72.11,i-0123...", key="add_input_right")
        ...
        sel_label = st.selectbox("Select server", options=labels, key="server_select_left_duplicate")
        st.markdown("<div style='margin-top:8px'></div>", unsafe_allow_html=True)
        st.button("Remove selected", on_click=remove_server_by_label, args=(sel_label,), key="remove_selected_right")
    ...
    with right_col:
        ...
        region = st.text_input("AWS region (used for SSM)", value=st.session_state.get("aws_region", "us-east-1"), key="aws_region_right")
        st.session_state.aws_region = region

        # Prominent query box
        st.subheader("Ask a question about the selected server")
        query = st.text_area("Enter your query", height=220, key="main_query_right")
        run = st.button("Run Query", key="run_query_right")

        agent_area = st.empty()
        # dedicated area for CPU summaries/charts so later UI writes don't overwrite it
        cpu_area = agent_area.container()
        ...
        if run:
            if not query or not query.strip():
                ...
                try:
                    ...
                    # recommended commands (profile-driven)
                    # use the local recommender (backup version)
                        ...
                        if paramiko is None:
                            ...
                            try:
                                ...
                                if out.get('Stdout'):
                                    ...
                                    # CPU parsing temporarily disabled to avoid parser errors
                                if out.get('Stderr'):
                            ...
                            if paramiko is None:
                                ...
                                try:
                                    ...
                                    if out.get('Stdout'):
                                        agent_area.markdown("**Stdout**")
                                        agent_area.code(out.get('Stdout'))
                                            # CPU parsing disabled in SSH execution path
                                    if out.get('Stderr'):
                                        agent_area.markdown("**Stderr**")
                                        agent_area.code(out.get('Stderr'))
                                except Exception as e:
